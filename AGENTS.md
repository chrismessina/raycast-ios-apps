...
- `ipatool` v2.6.0+ — Homebrew install required; auto-detected at common paths or set via `ipatoolPath` preference. v2.4.0 is where Apple's commerce auth gate was fixed: PR majd/ipatool#525 replaced App Store auth with SAP-signed requests, closing the HTTP 403 "empty or non-plist body" failures (majd/ipatool#522, #523). **v2.6.0 is the floor as of 2026-09-13**: Apple's redownload endpoint now answers older builds with an empty product payload (`Items: []`, surfaced as the bare `invalid response`, majd/ipatool#538) or an HTTP 500 non-plist body, so 2.5.0 can no longer download anything. 2.6.0 also writes the download to a sibling `<name>.ipa.tmp` and assembles the final `.ipa` from it only at the end (it injects iTunes artwork), which the progress poller in `src/ipatool.ts` has to watch — matching `.ipa` alone reports zero progress for the whole network phase. The version gate accepts any build ≥ 2.6.0.
